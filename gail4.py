...
def separate_gershgorin_circles(original_matrix, diagonal_matrix):
    # 通过对角矩阵和其逆矩阵乘积，分离盖尔圆
    separated_matrix = np.dot(diagonal_matrix, np.dot(original_matrix, np.linalg.inv(diagonal_matrix)))
    # print(separated_matrix)

    # 创建一个画布，包含两个子图
    fig, axes = plt.subplots(nrows=1, ncols=2, figsize=(12, 6))

    # 画出原始矩阵的盖尔圆
    plot_gershgorin_circles(original_matrix, axes[0], title="原始矩阵的盖尔圆")

    # 画出分离后的行盖尔圆
    plot_gershgorin_circles(separated_matrix, axes[1], title="分离后的盖尔圆")
